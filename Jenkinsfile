pipeline {
    ...
    stages {
        ...
        stage('Build docker image') {
            steps {
                sh "docker image build -t rajkumar207/netflix0723:latest ."
            }
        }
        stage('Trivy Scan') {
            steps {
                script {
                    sh "trivy image --format json -o trivy-report.json rajkumar207/netflix0723:latest"
                }
        ...
        stage('publish docker image') {
            steps {
                sh "docker image push rajkumar207/netflix0723:latest"
            }
        }
